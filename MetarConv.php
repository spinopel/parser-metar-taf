<?php
/*
## Transforming the decoding result of METAR
...
class MetarConv extends Metar
{
	...
	//echo "\n"."Порыв ветра, м/с"."\n";
	private function convGustSpeed($gust) {
		if (is_numeric($gust)) {
			$gust_chg = round($gust, 0);
		} else {
			$gust_chg = null;
		}
		return $gust_chg;
	...
	//echo "\n"."Видимость, км"."\n";
	private function convVisibility($dist, $str) {
		...
		//поиск значения "9999NDV", которое не учтено в родительском классе
		if (stripos($str, "9999NDV") !== false) {
			$dist_chg = 10;
		}					
		return $dist_chg;
	}
	
	//Clouds_height` SMALLINT(5) DEFAULT NULL
	//echo "\n"."Высота облаков, м"."\n";
	private function convCloudsHeight($height) {
		if ($height == 0) {
			$height_chg = null;
		} else {
			$height_chg = $height;
	...
	public function convParam() {
		...
		//Gust` tinyint(3) DEFAULT NULL
		//echo "\n"."Порыв ветра, м/с"."\n";
		$this->wind_gust_speed = $this->convGustSpeed($this->wind_gust_speed);
		...
		//Clouds_height` SMALLINT(5) DEFAULT NULL
		//echo "\n"."Высота облаков, м"."\n";
		$this->cloud_height = $this->convCloudsHeight($this->cloud_height);
